...
setwd("C:/Users/Erod/Desktop/Pomegranate/2020529_221_BF_Test_Image.tif") 
...
rodSurfaceArea <- function(r,l){(4*pi*(r^2)) + ((2*pi*r) * pi)}
...
data.Full.Mid <- data.Full.Combined %>% 
  group_by(Object_ID, Data_Type, Image) %>%
...
data.Full.Summary <- data.Full.Combined %>%
  group_by(Object_ID, Data_Type, Image, Experiment) %>%
